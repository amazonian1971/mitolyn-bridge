...
export default function MitolynBridgePage() {
  ...
  // CRO: Sticky bottom CTA when user scrolls
  ...
  // Simple trackers (FB Pixel, gtag, TikTok Pixel if loaded)
  ...
  // Check if email captured in this session
  useEffect(() => {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      // Variant handling: ?v=A or B or auto-randomize
  ...
  // Floating testimonials (random interval, random position)
  useEffect(() => {
  ...
  // Exit-intent: open gate if trying to leave
  useEffect(() => {
  ...
  const validateEmail = (val: string): boolean => {
    // Better regex; allows + and subdomains
    const re = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
    return re.test(val);
  };
  ...
  // Variant-based copy tweaks
  const headline =
    variant === 'A'
      ? 'Scientists Discover Cellular Energy Switch That Burns Fat 70% Faster After 40'
      : 'The Harvard-Backed “Cellular Energy Switch” Helping Women 40+ Reboot Metabolism Fast';

  const ctaText =
    variant === 'A' ? '🔓 Watch Free Scientific Presentation →' : '👉 Unlock the Free Video Now';
  ...
  return (
    <>
      ...
      {showSuccessPopup && (
        <div className="fixed inset-0 bg-black bg-opacity-60 z-[60] flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-sm w-full p-6 relative animate-bounceIn">
            ...
            <p className="text-center text-gray-700 mb-4 text-sm">Your mitochondrial health guide is being sent to:</p>

            <div className="bg-green-50 border-2 border-green-300 rounded-lg p-3 mb-4">
              <p className="text-center font-semibold text-green-800 text-sm">📧 {email}</p>
            </div>
            ...
            <div className="text-center">
              <p className="text-xs text-gray-600 mb-2">📬 Check your inbox in the next 2-5 minutes</p>
              <p className="text-xs text-gray-500">(Don't forget to check your spam folder!)</p>
            </div>

            <div className="mt-4 text-center">
              <p className="text-xs text-blue-600 animate-pulse">Redirecting to your video presentation...</p>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl max-w-md w-full p-6 relative animate-slideUp">
            ...
            <div className="flex items-center gap-2 mb-4">
              <div className="flex-1 h-1 rounded bg-gray-200 overflow-hidden">
                <div className={`h-1 bg-green-600 transition-all ${gateStep === 1 ? 'w-1/2' : 'w-full'}`} />
              </div>
              <span className="text-xs text-gray-600">Step {gateStep}/2</span>
            </div>
            ...
            {gateStep === 1 && (
              <div className="text-center">
                ...
                <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-2">Quick question before the video:</h2>
                <p className="text-gray-600 mb-6 text-sm">This helps personalize your results timeline.</p>

                <div className="text-left space-y-4">
                  <div>
                    <label className="text-sm font-semibold text-gray-800 mb-2 block">Your #1 goal</label>
                      ...
                      {['Lose weight', 'Boost energy', 'Hormone balance', 'Overall health'].map((g) => (
                        <button
                          ...
                          className={`text-xs md:text-sm px-3 py-2 rounded-lg border ${
                            userGoal === g ? 'bg-green-600 text-white border-green-600' : 'bg-white text-gray-700 border-gray-300'
                          }`}
                        >
                          {g}
                        </button>
                  ...
                  <div>
                    <label className="text-sm font-semibold text-gray-800 mb-2 block">Your age range</label>
                    <div className="grid grid-cols-3 gap-2">
                      {['35-44', '45-54', '55-64', '65+'].map((age) => (
                        <button
                          ...
                          className={`text-xs md:text-sm px-3 py-2 rounded-lg border ${
                            userAgeRange === age ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300'
                          }`}
                        >
                          {age}
                        </button>
                ...
                <button
                  ...
                  disabled={!userGoal || !userAgeRange}
                  className="w-full mt-6 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-bold text-lg py-4 rounded-lg transition-all transform hover:scale-105 disabled:opacity-50"
                >
                  Continue →
                </button>

                <p className="text-xs text-gray-500 mt-4">Takes 10 seconds. No credit card required.</p>
              </div>
            )}

            {gateStep === 2 && (
              <div className="text-center">
                <h2 className="text-xl md:text-2xl font-bold text-gray-900 mb-3">Unlock The Mitochondrial Breakthrough Video</h2>
                ...
                <div className="bg-green-50 rounded-lg p-4 mb-6">
                  <p className="text-sm font-semibold text-green-800 mb-2">🎁 You'll Also Get FREE:</p>
                ...
                <form onSubmit={handleEmailSubmit} className="space-y-3">
                  ...
                  <button
                    ...
                    } text-white font-bold text-lg py-4 rounded-lg transition-all transform hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed`}
                  >
                    {isSubmitting ? 'Unlocking Video...' : ctaText}
                  </button>
                </form>

                <p className="text-xs text-gray-500 mt-4">🔒 Your email is 100% secure. No spam, ever.</p>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-600">{floatingTestimonials[currentTestimonial].action}</p>
              <p className="text-xs text-gray-500">{floatingTestimonials[currentTestimonial].timeAgo}</p>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="min-h-screen bg-gradient-to-b from-gray-50 to-white">
          ...
          <div className="max-w-5xl mx-auto px-4">
            <div className="text-center mb-4">
              <span className="inline-flex items-center gap-2 bg-yellow-100 text-yellow-800 px-3 py-1 rounded-full text-xs md:text-sm font-semibold">
                <span>⚡</span> BREAKING: Harvard Study Validates Mitochondrial Breakthrough
              </span>
            </div>
            ...
            <div className="relative max-w-3xl mx-auto mb-6 md:mb-8">
              <div className="relative bg-gradient-to-br from-gray-900 to-gray-700 rounded-2xl overflow-hidden shadow-2xl aspect-video">
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <button onClick={handleCTAClick} className="relative group">
                ...
                <div className="absolute top-4 left-4 flex gap-3">
                  <span className="bg-gray-600 text-white px-3 py-1 rounded-full text-xs md:text-sm font-semibold">LOCKED</span>
                  <span className="bg-black bg-opacity-75 text-white px-3 py-1 rounded-full text-xs md:text-sm">12:47</span>
                </div>
              ...
              <button
                ...
                className={`w-full mt-4 ${
                  variant === 'A'
                    ? 'bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800'
                    : 'bg-gradient-to-r from-indigo-600 to-purple-700 hover:from-indigo-700 hover:to-purple-800'
                } text-white font-bold text-lg md:text-xl py-4 md:py-5 rounded-lg transition-all transform hover:scale-105 shadow-lg animate-pulse`}
              >
                {ctaText}
              </button>
            </div>
          ...
          <div className="max-w-5xl mx-auto px-4">
            ...
            <div className="grid md:grid-cols-2 gap-4 md:gap-6">
              {[
                ...
                <div key={i} className="bg-white rounded-xl shadow-lg p-4 md:p-6 border-t-4 border-green-500">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-xs md:text-sm text-gray-600">{t.location}</p>
                      <div className="flex text-yellow-400 text-sm md:text-base">★★★★★</div>
                    </div>
                  </div>
                  <div className="mb-4">
                    <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs md:text-sm font-bold">{t.result}</span>
                  </div>
                  <p className="text-gray-700 italic mb-4 text-sm md:text-base">"{t.quote}"</p>
        ...
        <section className="py-8 md:py-16 bg-white">
          <div className="max-w-4xl mx-auto px-4">
            <h2 className="text-2xl md:text-3xl font-bold text-center mb-8 text-gray-900">Special Pricing Available Today Only</h2>
            ...
            <div className="grid md:grid-cols-3 gap-4 mb-8">
			  ...
			  <span className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-green-500 text-white px-3 py-1 rounded-full text-xs font-bold">
                ...
                <p className="text-3xl font-bold text-blue-700">$294</p>
                <p className="text-sm text-gray-600">per bottle</p>
                <p className="text-sm text-gray-500 mt-2">180-day supply</p>
                <p className="text-xs text-blue-600 font-semibold mt-1">Save $117!</p>
              </div>
              ...
              <div className="border-2 border-blue-500 rounded-lg p-4 text-center bg-blue-50">
                ...
                <p className="text-3xl font-bold text-green-700">$177</p>
                <p className="text-sm text-gray-600">per bottle</p>
          ...
          <div className="max-w-4xl mx-auto px-4">
            <div className="bg-white rounded-2xl shadow-2xl p-6 md:p-8 lg:p-12">
              ...
              <div className="bg-red-50 border-2 border-red-300 rounded-lg p-4 md:p-6 mb-6 md:mb-8">
                ...
                <p className="text-center text-3xl md:text-4xl font-bold text-red-600 font-mono">{formatTime(timeLeft)}</p>
                <p className="text-center text-sm md:text-base text-red-700 mt-2">Only {spotsLeft} spots remaining today</p>
              </div>
              ...
              <button
                ...
                className={`w-full ${
                  variant === 'A'
                    ? 'bg-gradient-to-r from-gray-600 to-gray-700 hover:from-gray-700 hover:to-gray-800'
                    : 'bg-gradient-to-r from-indigo-600 to-purple-700 hover:from-indigo-700 hover:to-purple-800'
                } text-white font-bold text-lg md:text-2xl py-4 md:py-6 rounded-lg transition-all transform hover:scale-105 shadow-2xl animate-pulse`}
              >
                {ctaText}
              </button>

              <div className="mt-4 p-3 bg-yellow-50 border border-yellow-300 rounded-lg">
                <p className="text-center text-yellow-700 text-sm">🔒 Enter your email above to unlock instant access</p>
              </div>
        ...
        {!emailCaptured && showStickyCTA && (
          <div className="fixed bottom-0 inset-x-0 z-40 bg-white/90 backdrop-blur border-t shadow-lg">
            <div className="max-w-5xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                <span className="text-red-600">⏰</span>
                <span className="font-mono font-bold text-red-700">{formatTime(timeLeft)}</span>
                <span className="hidden sm:inline text-gray-600">• {spotsLeft} spots left</span>
              </div>
              <button
                onClick={() => {
                  setShowEmailModal(true);
          ...
          <div className="max-w-4xl mx-auto px-4 text-center text-xs text-gray-500 space-y-2">
            ...
            <div className="text-center text-xs text-gray-500 space-y-3 pt-4 md:pt-6">
              ...
              <p className="flex justify-center gap-4 md:gap-6">
                ...
                <a href="/disclaimer" className="underline hover:text-teal-600">
                  Affiliate Disclaimer
                </a>
                <a href="/scientific-references" className="underline hover:text-teal-600">
